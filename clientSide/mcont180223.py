...
class IR:
    ...
    def check(self):
        ...
        if GPIO.input(self.pin):
            ...
            self.hold = 0
            if (self.errCor and self.motor.dc != speed):
                print("%i" + ': Input was TRUE',speed)
            ...
            if(self.hold < 40):
                self.hold += 1
            #self.hold += 1
            if (self.errCor and self.motor.dc != speed - speedDelta):
                print("%i" + ': Input was TRUE',speed - speedDelta)
...
class MotorCoordinator:
    ...
    def __init__(self, evt_loop):
        ...
        self.IRL = IR(16, self.motor2,0)
        self.IRR = IR(21, self.motor1,0)
    ...
    def pollSensors(self):
        ...
        if (self.scanNodeRInv and  self.IRR.check()):
            print('reached node')
            self.motor1.stop()
            self.motor2.stop()
            self.scanNodeRInv = False
            self.inProg = False

        if (self.scanNodeL and not self.IRL.check()):
            print('reached node')
            self.motor1.stop()
            self.motor2.stop()
        ...
        if (self.scanNodeLInv and self.IRL.check()):
            print('reached node')
            self.motor1.stop()
            self.motor2.stop()
            self.scanNodeLInv = False
            self.inProg = False
    ...
    def setScanNodeRInv(self, scan):
        #self.scanNodeInv = scan
        self.schedule(self.setScanNodeRInvFunc, 0.1)

    def setScanNodeLInv(self, scan):
        #self.scanNodeInv = scan
        self.schedule(self.setScanNodeLInvFunc, 0.1)
    ...
    def setScanNodeRInvFunc(self):
        self.scanNodeRInv = True
        #self.schedule(self.scanNodeR = scan, 0.1)

    def setScanNodeLInvFunc(self):
        self.scanNodeLInv = True
        #self.schedule(self.scanNodeL = scan, 0.1)
...
class MotionCoordinator:
    ...
    def turnRightFwIndefInv(self):
        self.mc.setInProg()
        self.mc.ccw_motorForever(self.mc.motor1)
        self.mc.cw_motorForever(self.mc.motor2)
        self.mc.setScanNodeRInv(True)
        
    def turnRightFwIndefL(self):
        self.mc.setInProg()
        self.mc.ccw_motorForever(self.mc.motor1)
        self.mc.cw_motorForever(self.mc.motor2)
        self.mc.setScanNodeL(True)
        
    def turnRightFwIndefInvL(self):
        self.mc.setInProg()
        self.mc.ccw_motorForever(self.mc.motor1)
        self.mc.cw_motorForever(self.mc.motor2)
        self.mc.setScanNodeLInv(True)
    ...
    def turnLeftFwdIndef(self):
        self.mc.setInProg()
        self.mc.ccw_motorForever(self.mc.motor2)
        self.mc.cw_motorForever(self.mc.motor1)
        self.mc.setScanNodeL(True)
        
    def turnLeftFwdIndefInv(self):
        self.mc.setInProg()
        self.mc.ccw_motorForever(self.mc.motor2)
        self.mc.cw_motorForever(self.mc.motor1)
        self.mc.setScanNodeLInv(True)

    def turnLeftFwdIndefR(self):
        self.mc.setInProg()
        self.mc.ccw_motorForever(self.mc.motor2)
        self.mc.cw_motorForever(self.mc.motor1)
        self.mc.setScanNodeR(True)
        
    def turnLeftFwdIndefInvR(self):
        self.mc.setInProg()
        self.mc.ccw_motorForever(self.mc.motor2)
        self.mc.cw_motorForever(self.mc.motor1)
        self.mc.setScanNodeRInv(True)

    def turnLeftFw(self, start, end):
        print('turn right')
        self.mc.setInProg()
        self.mc.cw_motor(self.mc.motor1, start, end)
        self.mc.ccw_motor(self.mc.motor2, start, end)
...
class ActionCoordinator:
    ...
    def forward20ms(self):
        action = Action()
        action.add_step(self.mc.forward, 0.3)
        self.act(action)
    ...
    def backwardUntil(self):
        action = Action()
        self.mc.backwardUntil()
        # action.add_step(self.mc.forward, 0.2)
        # self.act(action)

    def backwardUntilNode(self):
        action = Action()
        self.mc.backwardUntilNode()

    def turnRightFwd(self):
        action = Action()
        #action.add_step(self.mc.forward, 0.2)
        action.add_step(self.mc.turnRightFw, 0.2)
        self.act(action)

    def turnLeftFwd(self):
        action = Action()
        #action.add_step(self.mc.forward, 0.2)
        action.add_step(self.mc.turnLeftFw, 0.2)
        self.act(action)
    ...
    def turnRightFwdIndefInv(self):
        action = Action()
        self.mc.turnRightFwIndefInv()
        
    def turnRightFwdIndefL(self):
        action = Action()
        self.mc.turnRightFwIndefL()
        
    def turnRightFwdIndefInvL(self):
        action = Action()
        self.mc.turnRightFwIndefInvL()
        
    def turnLeftFwdIndef(self):
        action = Action()
        self.mc.turnLeftFwdIndef()
        
    def turnLeftFwdIndefInv(self):
        action = Action()
        self.mc.turnLeftFwdIndefInv()

    def turnLeftFwdIndefR(self):
        action = Action()
        self.mc.turnLeftFwdIndefR()
        
    def turnLeftFwdIndefInvR(self):
        action = Action()
        self.mc.turnLeftFwdIndefInvR()
    ...
    def backwardStart(self):
        print('Backward One Node START')
        self.currentCommand = "Backward"

    def backwardEnd(self):
        print('Backward One Node END')
        self.currentCommand = ""
    ...
    def forwardLTurnStart(self):
        print('Left One Node START')
        self.currentCommand = "Left"

    def forwardLTurnEnd(self):
        print('Left One Node End')
        self.currentCommand = ""

    def backwardRTurnStart(self):
        print('Right back One Node START')
        self.currentCommand = "RightBack"

    def backwardRTurnEnd(self):
        print('Right back One Node End')
        self.currentCommand = ""

    def backwardLTurnStart(self):
        print('Left back One Node START')
        self.currentCommand = "LeftBack"

    def backwardLTurnEnd(self):
        print('Left back One Node End')
        self.currentCommand = ""
...
class CommandCoordinator:
    ...
    def bck1(self):
        ...
        self.act_queue.put('backwardStart')
        # self.commandStart()
        # 
        self.act_queue.put('backwardUntilNode')  # self.ac.forward(0.2)
        ## Change current position (in xml file) to in between current node and next node (Ex: Current Node = Between Prev Node & Next Node)
        self.act_queue.put('backwardUntil')  # self.ac.forwardUntil()
        ## Change current position (in xml file) to next node (Ex: Current Node = Next Node, Prev and Next node null)
        # self.act_queue.put('commandComplete') #self.commandComplete()
        self.act_queue.put('backwardEnd')
    ...
    def fwdRTurn(self):
        ...
        # Change current position to in between current node and next node
        #self.fwd1()
        self.act_queue.put('forwardUntilNode')
        self.act_queue.put('turnRightFwdIndef')
        self.act_queue.put('turnRightFwdIndefInv')  # self.ac.turnRightFwd()
        self.act_queue.put('turnRightFwdIndef')
        self.act_queue.put('turnRightFwdIndefInvL')
        self.act_queue.put('forward20ms')
        self.act_queue.put('turnRightFwdIndefInvL')
        self.act_queue.put('forward20ms')
        self.act_queue.put('turnRightFwdIndefInvL')
        self.act_queue.put('forward20ms')
        #self.act_queue.put('forwardUntil')
        #self.act_queue.put('turnRightFwdIndefInv')
        #self.act_queue.put('forwardUntilNode')
        #self.act_queue.put('turnRightFwdIndefInv')
        #self.act_queue.put('turnLeftFwdIndef')
        #self.fwd2()
        # self.commandComplete()
    ...
    def fwdLTurn(self):
        # Go Backward one node
        self.act_queue.put('forwardLTurnStart')
        # Change current position to in between current node and next node
        #self.fwd1()
        self.act_queue.put('forwardUntilNode')
        self.act_queue.put('turnLeftFwdIndef')
        self.act_queue.put('turnLeftFwdIndefInv')  # self.ac.turnRightFwd()
        self.act_queue.put('turnLeftFwdIndef')
        self.act_queue.put('turnLeftFwdIndefInvR')
        self.act_queue.put('forward20ms')
        self.act_queue.put('turnLeftFwdIndefInvR')
        self.act_queue.put('forward20ms')
        self.act_queue.put('turnLeftFwdIndefInvR')
        ...
        #self.fwd2()
        # self.commandComplete()
        self.act_queue.put('forwardLTurnEnd')

    def bckRTurn(self):
        # Go Backward one node
        self.act_queue.put('backwardRTurnStart')
        # Change current position to in between current node and next node
        #self.fwd1()
        self.act_queue.put('backwardUntilNode')
        self.act_queue.put('turnLeftFwd')
        self.act_queue.put('turnLeftFwdIndef')  # self.ac.turnRightFwd()
        self.act_queue.put('backwardUntilNode')
        #self.act_queue.put('turnRightFwdIndefInv')
        #self.act_queue.put('turnLeftFwdIndef')
        #self.fwd2()
        # self.commandComplete()
        self.act_queue.put('backwardRTurnEnd')

    def bckLTurn(self):
        # Go Backward one node
        self.act_queue.put('backwardLTurnStart')
        # Change current position to in between current node and next node
        #self.fwd1()
        self.act_queue.put('backwardUntilNode')
        ...
        #self.fwd2()
        # self.commandComplete()
        self.act_queue.put('backwardLTurnEnd')
    ...
    def run(self):
        while True:
            ...
            if (not self.cc.act_queue.empty()) and (not self.cc.ac.mc.inProg):
                time.sleep(0.2)  ## Sleep for testing purposes
...
if __name__ == '__main__':
    ...
    SERVER_MAC_ADDRESS = '00:1A:7D:DA:71:13' # My bluetooth
